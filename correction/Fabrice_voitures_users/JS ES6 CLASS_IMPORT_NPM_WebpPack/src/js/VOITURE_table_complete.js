...
// Import notre CSS qui contient SCSS BS
import "../scss/VOITURE_table_complete.scss";
...
function supprVoiture(e) {
  ...
  Swal.fire({
    ...
    denyButtonText: `Annuler la suppression`,
  }).then((result) => {
    /* Read more about isConfirmed, isDenied below */
    if (result.isConfirmed) {
      Swal.fire("Voiture supprimée !", "", "success");
      Obj_AJAX_voitures.Cle = valeurs[0]; // Recupération du code à supprimer
      Obj_AJAX_voitures.del(retourAjaxSuppr, erreur);
    } else if (result.isDenied) {
      Swal.fire("Suppression abandonnée", "", "info");
    }
  });
}
