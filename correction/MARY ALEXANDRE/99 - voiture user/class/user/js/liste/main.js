...
function modifStag(e){
    ...
    document.getElementById('cylindree_vue').value = valeurs[3];
      ...
      function RetourAjax(reponse) {
        ...
        tab.header = myObj.users.columns;
        tab.data = myObj.users.records;
      ...
      function retourAjout(reponse) {
        document.getElementById('balert').innerHTML = '<div id="balert" class="alert alert-success" role="alert">Utilisateur correctement ajouté code : ' + reponse + ' !</div>';
        // Liste des voitures
        Afficher_Liste();
      }
      ...
      function retourSuppr(reponse) {
        document.getElementById('balert').innerHTML = '<div id="balert" class="alert alert-danger" role="alert">Utilisateur correctement supprimé !</div>'
        // Liste des voitures
        Afficher_Liste();
      }
      ...
      function affichage() {
      document.getElementById('balert').innerHTML = '<div class="alert alert-info" role="alert">Voici la liste des utilisateur</div>'
    }
    ...
    function erreursupp(){
      document.getElementById('balert').innerHTML = '<div class="alert alert-success" role="alert">Utilisateur Supprimé</div>'
    }
      ...
      window.addEventListener('load', (event) => {
        ...
        document.getElementById('btn-ajout').addEventListener('click', () => {
          ...
          document.getElementById('balert').innerHTML = '<div id="balert" class="alert alert-danger" role="alert">Utilisateur tous les champs !</div>'
        } else {
          if (emailSaisi.includes("@") && emailSaisi.includes(".")) {
            let positionArobase = emailSaisi.indexOf("@");
            let emailSaisiCoupe =  emailSaisi.substring(positionArobase);
            if (emailSaisiCoupe.includes(".")) {
              let data = {};
              data.nom = document.getElementById('marque_ajout').value;
              data.prenom = document.getElementById('couleur_ajout').value;
              data.email = document.getElementById('cylindreev').value;
              console.log("Les valeurs envoyées : " + JSON.stringify(data));
              document.getElementById('balert').innerHTML = ""
              document.getElementById('balert').innerHTML = '<div id="balert" class="alert alert-indo" role="alert">Utilisateur ajouté !</div>'
              Obj_AJAX.post(JSON.stringify(data), retourAjout, erreur);
            } else {
              message.innerHTML = '<div id="balert" class="alert alert-danger" role="alert">Email Invalide !</div>'
      ...
      document.getElementById('btnmodif').addEventListener('click', ()=>{
          let data = {};
          data.nom = document.getElementById('marque_modif').value;
          data.prenom = document.getElementById('couleur_modif').value;
          data.email = document.getElementById('cylindree_modif').value;    
